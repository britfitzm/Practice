...
int handle_cant_open(char *file_path);
int process_file_commands(char *file_path, int *exe_ret);

/**
 * handle_cant_open - If the file doesn't exist or lacks proper permissions, print
 * a cant open error.
...
int handle_cant_open(char *file_path)
{
	...
	len = _strlen(shell_name) + _strlen(hist_str) + _strlen(file_path) + 16;
...
int process_file_commands(char *file_path, int *exe_ret)
{
	...
	if (file == -1)
	{
		*exe_ret = handle_cant_open(file_path);
		return (*exe_ret);
	}
	...
	perform_variable_replacement(&line, exe_ret);
